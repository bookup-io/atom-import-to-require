...
export default {
  ...
  activate(state) {
    ...
    this.subscriptions.add(atom.commands.add('atom-workspace', {
      ...
      'atom-add-translation:convertNoBracket': () => this.convertNoBracket(),
      'atom-add-translation:convertFirstNoBracket': () => this.convertFirstNoBracket(),
      'atom-add-translation:convertAllNoBracket': () => this.convertAllNoBracket(),
    }));
  },
  ...
  convert() {
    ...
    const splitIfTooLong = (string, selection) => {
      if (string.length > 50) {
        ...
        .replace('intl.formatMessage(messages.', '')
        .replace('messages.', '')
        .replace(')}', '')
        .replace(/'/g, '')
        .replace(/{/g, '')
